...
- [The Stall](https://github.com/thebrierfox/the-stall) - Remote Streamable HTTP MCP with OFAC sanctions screening (19,000+ SDN entries, fuzzy name match and AKA alias lookup), wallet risk and credit scoring, agent KYA trust scoring, EVM and Solana token security analysis, and EVM transaction intelligence; no API key required, x402 USDC pay-per-call on Base at `https://the-stall.intuitek.ai/mcp`.
