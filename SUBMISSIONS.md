...
- **Hosted MCP crawler compatibility:** Fix the public crawler path before
  ...
  MIT). The latest source build/tests/compliance path passes, so the remaining
  blocker is the hosted deployment plus Cloudflare crawler/runtime path, not a
  public metadata mismatch.
