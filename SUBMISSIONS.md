...
| [mcpservers.org / wong2/awesome-mcp-servers](https://mcpservers.org/submit) | High-traffic MCP directory generated from `wong2/awesome-mcp-servers`; its README already had a stale Hive link pointing at `hive-crypto-mcp`, and its GitHub repo auto-closes PRs in favor of website submissions. | Wait for the submitted canonical listing/update to finish replacing the stale repo shell; avoid opening a GitHub PR because upstream auto-closes in favor of website submissions. | Live listing: https://mcpservers.org/servers/hiveintelligencexyz/hive-mcp. Submitted canonical update on 2026-06-01 with id `2916`; submitted canonical `Hive Intelligence MCP` listing with direct `hive-sdk` URL on 2026-06-01, id `2919`, category `finance`, status `pending`. Rechecked on 2026-06-02: the page now includes `hive-intelligence`, `https://mcp.hiveintelligence.xyz/mcp`, `hive-intel/hive-sdk` support links, and 369-tool public copy, but the primary GitHub button still points at `hive-intel/hive-crypto-mcp`, so this is only partially refreshed. |
