...
| [Glama MCP Servers](https://glama.ai/mcp/servers/hive-intel/hive-sdk) | MCP metadata and discovery directory. | `hive-sdk` already has `glama.json`; rerun Glama's claim ownership flow so Glama refreshes the listing copy from the current file. | Listed with score page; Glama API still shows older description as of 2026-06-01, so copy needs claim refresh. |
