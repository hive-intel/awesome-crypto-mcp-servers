...
| [TensorBlock/awesome-mcp-servers](https://github.com/TensorBlock/awesome-mcp-servers) | Broad MCP directory with a Finance & Crypto section and hundreds of active entries. | Keep Hive Intelligence listed and canonicalize the repo URL to `hive-sdk` instead of stale `hive-crypto-mcp`. | Hive is listed; PR open at https://github.com/TensorBlock/awesome-mcp-servers/pull/623. Active PR refreshed on 2026-06-01 to replace the stale repo path, deprecated Hive install/setup links, and old narrow market-infrastructure copy with canonical `hive-sdk`, `hive-intelligence`, hosted MCP, SDK, skills, and workflow coverage. Maintainer requested the mirrored `docs/finance--crypto.md` update; branch was updated and a re-review comment was posted on 2026-06-02. |
| [royyannick/awesome-blockchain-mcps](https://github.com/royyannick/awesome-blockchain-mcps) | Blockchain-specific MCP list that already had a stale Hive entry. | Keep the entry pointed at `hive-sdk` with current hosted MCP, local stdio, SDK, and skills positioning. | PR open: https://github.com/royyannick/awesome-blockchain-mcps/pull/65. Follow-up note posted on 2026-06-02 clarifying the canonical repo and managed crypto intelligence positioning: https://github.com/royyannick/awesome-blockchain-mcps/pull/65#issuecomment-4596494600 |
| [badkk/awesome-crypto-mcp-servers](https://github.com/badkk/awesome-crypto-mcp-servers) | Crypto-specific MCP list with overlap in target developer audience. | Add Hive Intelligence with concise managed crypto MCP positioning. | PR open: https://github.com/badkk/awesome-crypto-mcp-servers/pull/84. Follow-up note posted on 2026-06-02 clarifying the canonical `hive-sdk`, hosted MCP, and `hive-intelligence` package surface: https://github.com/badkk/awesome-crypto-mcp-servers/pull/84#issuecomment-4596494553. Older duplicate PR https://github.com/badkk/awesome-crypto-mcp-servers/pull/57 closed as superseded on 2026-06-01. |
| [demcp/awesome-web3-mcp-servers](https://github.com/demcp/awesome-web3-mcp-servers) | Web3-specific MCP list and one of the cleanest category-fit discovery surfaces for Hive. | Do not open a duplicate. Keep the active Hive PR warm and resolve maintainer feedback if review resumes. | PR open but review-blocked: https://github.com/demcp/awesome-web3-mcp-servers/pull/78. Active PR refreshed on 2026-06-01 to point at canonical `hive-sdk` and current managed crypto MCP positioning; follow-up note posted on 2026-06-02: https://github.com/demcp/awesome-web3-mcp-servers/pull/78#issuecomment-4596494566. Older stale duplicate from 2025-09-10 remains open at https://github.com/demcp/awesome-web3-mcp-servers/pull/10; a second close attempt on 2026-06-02 was still blocked by GitHub permissions, so a correction comment now points maintainers to #78 and `hive-intel/hive-sdk`: https://github.com/demcp/awesome-web3-mcp-servers/pull/10#issuecomment-4595601702. |
...
| [MCPRepository](https://mcprepository.com/hive-intel/hive-sdk) | GitHub-indexed MCP server directory with server pages, search, and an npm-backed submission CLI. | Refresh the existing Hive listing from current `hive-sdk` README metadata so it uses the canonical support repo, 369-tool public copy, and current `hive-intelligence` CLI path. | Listing exists. Re-submission returned `{"status":"check","valid":true,"duplicate":true}` on 2026-06-01; stale metadata refresh issue submitted: https://github.com/mcprepository/mcp-index/issues/1. Rechecked on 2026-06-02: page still mixes canonical `hive-sdk`, `hive-intelligence`, and hosted MCP setup with stale `hive-crypto-mcp`, `hiveintelligence.xyz/crypto-mcp`, and `376+ live tools` copy. |
| [MCPDrops](https://www.mcpdrops.com/mcp/hive) | Crypto-heavy MCP marketplace and review surface that already has a Hive listing and several stale deprecated Hive category entries. | Replace old `hive-crypto-mcp`, `mcp-hive`, `200+ tools`, and deprecated `crypto-mcp-*` category surfaces with the canonical `hive-sdk`, hosted MCP, npm package, and 369-tool public contract. | Source remediation merged in private MCPDrops PR #1 on 2026-06-01. Rechecked on 2026-06-02: the live page still has `hive-crypto-mcp`, `mcp-hive`, and `200+` tool copy; no canonical `hive-sdk`, hosted MCP URL, or 369-tool copy detected. |
| [MCP.so](https://mcp.so/server/hive-intelligence) | Large MCP directory with indexed server pages, category pages, and search visibility across "Claude MCP" and "Awesome MCP Servers" queries. | Refresh the stale Hive listing so it points at `hive-sdk`, `hive-intelligence`, and `npx -y -p hive-intelligence@latest hive` instead of the deprecated `hive-crypto-mcp` / `mcp-hive` path. | Issue submitted: https://github.com/chatmcp/mcpso/issues/2597. Rechecked on 2026-06-02: the live page still contains `hive-intel/hive-crypto-mcp`, `mcp-hive`, and `200+` tool copy, while no canonical `hive-intel/hive-sdk` reference was detected. Direct API update requires login. |
...
| [MCPList.ai](https://www.mcplist.ai/) | Broad MCP directory claiming 775+ verified servers, daily updates, and 10,000+ developer usage; useful for generic "MCP directory" discovery rather than crypto-specific search. | Request inclusion or refresh for Hive Intelligence with canonical `hive-sdk`, `hive-intelligence`, hosted MCP URL, and Finance/Data/Security positioning; submit this awesome list only if they accept curated lists as resources. | New target identified on 2026-06-01. Rechecked on 2026-06-02: search markup includes `Awesome Crypto MCP Servers by badkk`, but does not show Hive Intelligence, `hive-intel/hive-sdk`, `hive-intel/awesome-crypto-mcp-servers`, or `hive-intelligence`. No public source repo or machine-readable submit endpoint found; contribution copy points to contact/submission form or GitHub repository, while About lists `contact@mcplist.ai`. |
...
| [LobeHub MCP Marketplace](https://lobehub.com/en/mcp/hiveintelligencexyz-hive-crypto-mcp) | High-traffic agent and MCP marketplace with localized pages and one-click install flows. It indexed stale Hive pages pointing at deprecated repos and endpoints. | Monitor search/cache refresh and resubmit only if canonical Hive disappears or stale category-level pages remain live after the marketplace refresh. | Issue closed as resolved on 2026-06-01: https://github.com/lobehub/lobehub/issues/15361. A LobeHub contributor said outdated Hive listings were removed so users are directed to the maintained listing. Rechecked on 2026-06-02: the stale `hiveintelligencexyz-hive-crypto-mcp` URL now returns 404, so this cleanup appears effective; keep monitoring for canonical Hive search visibility. |
...
- **Hosted MCP crawler compatibility:** Fix the public crawler path before
  ...
  Cloudflare `403`. The website server card is live and now has canonical
  public metadata at
  `https://www.hiveintelligence.xyz/.well-known/mcp/server-card.json`
  (`xyz.hiveintelligence/mcp`, `hive-intel/hive-sdk`, `hive-intelligence`,
  MIT). The latest source build/tests/compliance path passes; the latest
  deployment is blocked before rollout because GCP billing prevents Artifact
  Registry image push. After billing is restored and the deployment is rerun,
  recheck Cloudflare crawler/runtime routing separately. This is not a public
  metadata mismatch.
