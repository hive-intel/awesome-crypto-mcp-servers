...
| Glama MCP Servers | MCP metadata and discovery directory at `glama.ai/mcp/servers`. | Rerun Glama's claim/refresh flow so it recrawls `hive-sdk` and evaluates the Docker-backed stdio surface. | Listed at slug `hive-intel/hive-sdk`. Remediation merged in `hive-sdk` PR https://github.com/hive-intel/hive-sdk/pull/10: Dockerfile, stdio wrapper, and optional `HIVE_API_KEY` metadata. Glama API still shows older description, empty env schema, and no tools as of 2026-06-01; needs recrawl. |
