...
| [MCP Registry](https://registry.modelcontextprotocol.io/) | Official MCP server discovery registry now preferred over README PRs to `modelcontextprotocol/servers`. | Publish `hive-sdk/server.json` with `mcp-publisher` after registry auth is available. The current `xyz.hiveintelligence/mcp` name likely requires domain verification for `hiveintelligence.xyz`. | Needs authenticated publish. |
